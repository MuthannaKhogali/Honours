      ...
      <main class="container-fluid mt-4 px-3">
        ...
        <div v-if="questions.length" class="shadow-lg p-4 mb-4 bg-white rounded">
          <template v-if="!quizFinished">
            ...
            <input v-model="selectedOption" class="form-control" type="text" placeholder="Type your answer here" 
              :disabled="answeredQuestions.includes(currentQuestion) || checkingAnswer" />
          ...
          <template v-if="quizFinished">
            ...
            <p :class="{
    'text-success': answers[index]?.feedback?.trim().toLowerCase() === 'correct!',
    'text-danger': answers[index]?.feedback?.trim().toLowerCase() === 'incorrect!'
}">
    ...
    export default {
      ...
      methods: {
        ...
        try {
            ...
            // Save both the user's answer and feedback in the answers array
            ...
            this.answeredQuestions.push(this.currentQuestion); // 🔹 Mark as answered
            } catch (error) {
